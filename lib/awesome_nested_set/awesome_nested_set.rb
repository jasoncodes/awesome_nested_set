        ...
        module ClassMethods
          ...
          def rebuild!(validate_nodes = true)
            ...
            set_left_and_rights = lambda do |node|
              ...
              # find
              where(["#{quoted_parent_column_name} = ? #{scope.call(node)}", node]).order("#{quoted_left_column_name}, #{quoted_right_column_name}, #{primary_key}").each{|n| set_left_and_rights.call(n) }
              # set right
            ...
            # Find root node(s)
            root_nodes = where("#{quoted_parent_column_name} IS NULL").order("#{quoted_left_column_name}, #{quoted_right_column_name}, #{primary_key}").each do |root_node|
